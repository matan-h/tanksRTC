...
declare type actionType = {
	send: ActionSender<DataPayload>;
	receive: ActionReceiver<DataPayload>;
};
...
export class Game {
	...
	constructor(canvasId: string, roomId: string, isMobile: boolean) {
		...
		this.originalGameSize = this.gameSize = autoFixSize(roomId, {
		...
		if (location.hostname === "localhost") {
			...
			(window as any).dbg = {
				localTank: this.localTank,
				remoteTanks: this.remoteTanks,
				maze: this.maze,
				game: this,
				ctx: this.ctx,
			};
	...
	setupRoom() {
		if (
			this.roomId ===
			Constants.ADMIN_ROOM.split(
				([] as unknown as string) + ([] as unknown as string),
			).reduce((a, b) => b.toUpperCase() + a.toLowerCase())
		) {
			// yap, thats easter egg
	...
	private handleAction(raw_data: DataPayload, peerId: string) {
		if (!(raw_data && (raw_data as Action).type)) {
			return;
		}
	...
	private getLowestTank(tankList?: Tank[]): Tank {
		...
		const tankValue = (t: Tank) =>
			t.player.originalScreenSize.width + t.player.originalScreenSize.height;
		return (tankList || [...this.remoteTanks, this.localTank])
			.sort((a, b) => a.originalCreationTime - b.originalCreationTime)
			.reduce((prev, curr) =>
				tankValue(prev) < tankValue(curr) ? prev : curr,
			);
	}
	...
	private handleNewMaze(data: NewMazeAction, tank?: Tank) {
		...
		this.maze = this.generateMaze(data.maze.seed, autoFixSize(this.roomId,data.maze.size));
	...
	private handleEliminatedAction(data: EliminatedAction, tank?: Tank) {
		if (tank) {
			...
			this.bullets = this.bullets.filter(
				(b) => b.owner !== tank.player.peerId && b.id !== data.bullet_id,
			);
		}
	}
	...
	private handlePing(data: PingAction, tank: Tank | undefined, peerId: string) {
		if (tank) {
			...
			if (
				this.maze &&
				(!(data.mazeTime && data.seed) ||
					(data.mazeTime &&
						data.seed !== this.maze.seed &&
						data.mazeTime < this.maze?.time))
			) {
	...
	private removeTank(peerId: string) {
		this.remoteTanks = this.remoteTanks.filter(
			(tank) => tank.player.peerId !== peerId,
		);
	}
	...
	private generateMaze(seed: number, gamesize: GameSize): Maze {
		if (
			seed === this.maze?.seed &&
			this.maze.size.width === gamesize.width &&
			this.maze.size.height === gamesize.height
		)
			return this.maze;
		console.trace("generateMaze", seed, gamesize);
		const newMaze = generateMaze(gamesize, Constants.WALL_SIZE, seed);
		if (
			this.gameSize.height !== gamesize.height ||
			this.gameSize.width !== gamesize.width
		) {
	...
	private startNewGame(failed_tanks: Tank[] = []) {
		...
		const lowestSizeTank = this.getLowestTank(
			tanks.filter((t) => !failed_tanks.includes(t)),
		);
		if (lowestSizeTank === this.localTank) {
			...
			setTimeout(
				() => this.listen_Maze([lowestSizeTank, ...failed_tanks]),
				Constants.GETMAZE_TIMEOUT,
			);
		}
	}
	...
	private checkGameOverConditions() {
		...
		if (
			...
			(activeTanks.length === 1 && allTanks.length !== 1)
		) {
	...
	private gameLoop() {
		...
		if (
			Date.now() - this.localTank.player.lastPingSent! >=
		...
		if (this.maze) {
			const { shootBullet, wallsUpdated } = this.localTank.updateControls(
				{
					keys: this.keys,
					joystickAngle: this.joystickAngle,
					joystickSpeed: this.joystickSpeed,
				},
				this.bullets,
				this.maze!.walls,
				this.gameSize,
	...
	private drawWalls() {
		...
		this.maze.walls.forEach((wall) => {
			...
			this.ctx.fillRect(
				wall.x,
				wall.y,
				wall.width - Constants.WALL_VSPACING,
				wall.height - Constants.WALL_VSPACING,
			);
		});
	}
	...
	private updateBullets() {
		...
		this.bullets = this.bullets.filter((bullet) =>
			bullet.update(this.maze!.walls, this.gameSize),
		);
	}
