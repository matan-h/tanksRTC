...
export class Tank {
	...
	updateControls(
		...
		if (joystickSpeed > 0) {
			angular_difference =
				(((angular_difference + Math.PI) % (2 * Math.PI)) - Math.PI) / 10;
		...
		if (
			!this.checkWallCollision(newX, newY, walls) ||
			this.checkWallCollision(this.x, this.y, walls)
		) {
			if (!OOB.isOut) {
				this.angle = angle;
				this.x = newX;
		...
		if (now - this.lastShotTime > Constants.SHOOT_COOLDOWN) {
			if (this.getOwnBullets(bullets).length < Constants.TANK_BULLETS_MAX) {
				const bullet = new Bullet(
					...
					this.x +
						Math.cos(this.angle) *
							(Constants.TANK_SIZE / 2 + Constants.TURRET_SIZE),
					this.y +
						Math.sin(this.angle) *
							(Constants.TANK_SIZE / 2 + Constants.TURRET_SIZE),
	 ...
	 */
	private howOutOfBounds(
		newX: number,
		newY: number,
		angle: number,
		size: GameSize,
	): { outPos: { x: number; y: number }; isOut: boolean } {
		...
		return {
			outPos: { x: retX, y: retY },
			isOut: adjustX !== 0 || adjustY !== 0,
		};
	}
	...
	checkCollisionWithBullet(bullet: Bullet): boolean {
		...
		return (
			pointInRotatedRectangle(bullet.x, bullet.y, rectCorners) ||
			isPointInTriangle(
				bullet.x,
				bullet.y,
				turretVertices[0],
